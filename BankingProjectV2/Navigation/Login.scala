...
import BankingProjectV2.Accounts.CreditAccount
...
object Login extends App {
  ...
  def LoginProcedure(): Unit = {
    ...
    val accountDatabase = CSVReader.open(new File("src/main/scala/BankingProjectV2/AccountStorage/accountDatabase.csv"))
    ...
    do {
      ...
      Entry match {
        ...
        case 4 =>
          ...
          //the CSV
          var writer = CSVWriter.open("src/main/scala/BankingProjectV2/AccountStorage/accountDatabase.csv", append = true)
          var listBuffer = ListBuffer[Any]()
          //For each account owned by the user
          for (accounts <- obj.accountPortfolio) {
            if(accounts.AccountType != "Credit Account") {
              ...
              listBuffer += "Credit Account"
              for(cCard <- accounts.creditCards)
              listBuffer += (cCard.cardType, cCard.ccBalance, cCard.creditLimit, cCard.interestRate)
            }
          }
